...
def choose_base_strategy(total_pages: int) -> tuple[str, int, int, str]:
    if total_pages <= 40:
        return (
            ...
            max(1, total_pages),
            1,
            "The PDF is short enough to transform into a complete result in one pass.",
        )
    if total_pages <= 150:
        return (
            "segmented_read",
            10,
            3,
            "The PDF is medium-length, so segmented reading with periodic checkpoint summaries is recommended.",
        )
    if total_pages <= 400:
        return (
            "long_form_read",
            12,
            4,
            "The PDF is long, so close reading in segments is safer than loading too much text at once.",
        )
    return (
        "book_mode",
        15,
        5,
        "The PDF is very long and is better handled like an ebook with long-range segmented reading.",
    )
...
def build_plan(
    ...
    if persisted_state and persisted_state.get("segments"):
        ...
        if isinstance(persisted_segments, list) and persisted_segments:
            ...
            strategy_reason = "A saved session was detected, so the existing segmentation strategy was reused."
    ...
    if existing_session and not requires_ocr:
        ...
        recommended_action = (
            f"A saved session was found: currently on segment {current_segment} "
            f"(pages {current_page_start}-{current_page_end}) with {completed_count} "
            "completed segments. Resume the existing session first; if you want to restart, run init --force-reset."
        )
    elif strategy_mode == "single_pass":
        recommended_action = "Extract the full text directly and turn it into a complete result."
    elif requires_ocr:
        recommended_action = "Confirm whether this PDF requires OCR before deciding whether to continue with segmented reading."
    else:
        recommended_action = (
            f"Read segment 1 first (pages {segments[0]['page_start']}-{segments[0]['page_end']}), "
            "then use the session state to continue with the next segment."
        )
