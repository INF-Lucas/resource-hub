...
def checkpoint_note(
    ...
    if checkpoint is None:
        return "All checkpoint summaries have already been completed."

    missing = checkpoint["missing_segments"]
    if not missing:
        return (
            f"Checkpoint summary {checkpoint['checkpoint_index']} "
            f"(segments {checkpoint['start_segment']}-{checkpoint['end_segment']}) is ready to generate."
        )

    missing_text = ", ".join(str(segment) for segment in missing)
    if len(contiguous_segments) < len(completed_segments):
        return (
            f"{len(completed_segments)} segments have been completed so far, but the earliest "
            f"pending checkpoint is still segments {checkpoint['start_segment']}-{checkpoint['end_segment']}. "
            f"Missing segments: {missing_text}."
        )

    return (
        f"The nearest checkpoint summary is checkpoint {checkpoint['checkpoint_index']} "
        f"(segments {checkpoint['start_segment']}-{checkpoint['end_segment']}). "
        f"Missing segments: {missing_text}."
    )
...
def summarize_state(state_file: str) -> dict[str, Any]:
    ...
    summary = {
        ...
        "human_summary": (
            f"Currently on segment {current_segment}/{total_segments} "
            f"(pages {state['current_page_start']}-{state['current_page_end']}), "
            f"with {len(completed_segments)} completed segments "
            f"(about {overall_progress_percent:.1f}%)."
            + (
                f" Contiguous progress from the beginning reaches segment {contiguous_through_segment}."
                if contiguous_through_segment > 0
                else ""
            )
            + (
                " The current segment result has been saved."
                if current_segment_saved
                else " The current segment result has not been saved yet."
            )
            + (
