...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manage EPUB reading session state.")
    parser.add_argument("book_dir", help="Book output directory")
    parser.add_argument("command", choices=["get", "set", "clear"], help="Operation type")
    parser.add_argument("--mode", type=str, help="Set the current mode")
    parser.add_argument("--chapter", type=str, help="Set the current chapter ID")
    parser.add_argument("--chunk", type=int, help="Set the current chunk index")
    parser.add_argument("--action", type=str, help="Set the last action")
    parser.add_argument("--query", type=str, help="Set the last query")
...
def main() -> int:
    ...
    try:
        ...
        if not book_dir.exists():
            print(f"Error: directory does not exist: {book_dir}", file=sys.stderr)
            return 1
