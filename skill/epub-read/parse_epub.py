...
def detect_complex_content(
    ...
    if has_images:
        summary_parts.append(f"Contains {len(image_items)} image resources")
    if has_svg:
        summary_parts.append("Contains SVG vector graphics")
    if has_tables:
        summary_parts.append("Contains tables")
    if image_heavy_sections:
        summary_parts.append(f"Has {len(image_heavy_sections)} image-heavy chapters")
    if low_text_resource_sections:
        summary_parts.append(f"Has {len(low_text_resource_sections)} low-text high-resource chapters")

    summary = "; ".join(summary_parts) if summary_parts else "No notable complex content"
...
def build_parse_guidance(summary: dict[str, Any]) -> str:
    lines = [
        "EPUB parsing is complete. The following task types are now available:",
        "",
        "A. Quick overview: inspect the table of contents, structure, metadata, and themes",
        "B. Targeted chapter reading: read by chapter, TOC item, or chunk range",
        "C. Full-book reading: read the whole book sequentially by chunk",
        "D. Targeted extraction: extract keywords, concepts, quotes, examples, action items, and more",
        "E. Image and complex-content assist: inspect images, tables, and complex layouts",
        "F. Batch processing: batch-parse or batch-extract multiple EPUB files",
        "",
